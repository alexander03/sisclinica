...
<script>
	...
	function cambiarOrigen(value,id){
        $.ajax({
	        type: "POST",
	        url: "producto/cambiarOrigen",
	        data: "origen_id="+value+"&id="+id+"&_token="+$(IDFORMBUSQUEDA + '{!! $entidad !!} :input[name="_token"]').val(),
	        success: function(a) {
	            //buscar('{{ $entidad }}');
	        }
	    });
    }
    function cambiarAnaquel(value,id){
        $.ajax({
	        type: "POST",
	        url: "producto/cambiarAnaquel",
	        data: "anaquel_id="+value+"&id="+id+"&_token="+$(IDFORMBUSQUEDA + '{!! $entidad !!} :input[name="_token"]').val(),
	        success: function(a) {
	            //buscar('{{ $entidad }}');
	        }
	    });
