...
<script>
    ...
    function excel(entidad){
	    window.open("cita/excel?fecha="+$(IDFORMBUSQUEDA + '{{ $entidad }} :input[id="fecha"]').val()+"&doctor="+$(IDFORMBUSQUEDA + '{{ $entidad }} :input[id="doctor"]').val()+"&paciente="+$(IDFORMBUSQUEDA + '{{ $entidad }} :input[id="paciente"]').val()+"&_token="+$(IDFORMBUSQUEDA + '{!! $entidad !!} :input[name="_token"]').val(),"_blank");
	}
</script>