...
<script>
    ...
    function excel(entidad){
        window.open("facturacion/excel?fechainicial="+$(IDFORMBUSQUEDA + '{!! $entidad !!} :input[name="fechainicial"]').val()+"&fechafinal="+$(IDFORMBUSQUEDA + '{!! $entidad !!} :input[name="fechafinal"]').val()+"&numero="+$(IDFORMBUSQUEDA + '{!! $entidad !!} :input[name="numero"]').val()+"&paciente="+$(IDFORMBUSQUEDA + '{{ $entidad }} :input[id="paciente"]').val()+'&usuario='+$(IDFORMBUSQUEDA + '{{ $entidad }} :input[id="usuario"]').val(),"_blank");
    }
