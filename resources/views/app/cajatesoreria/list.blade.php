...
@else
    ...
    {!! Form::button('<i class="glyphicon glyphicon-remove-circle"></i> Cierre', array('class' => 'btn btn-danger btn-xs', 'id' => 'btnCerrar', 'onclick' => 'modalCaja (\''.URL::route($ruta["cierre"], array('listar'=>'SI')).'\', \''.$titulo_cierre.'\', this);')) !!}
@endif
{!! Form::button('<i class="glyphicon glyphicon-cog"></i> Actualizar Cajas', array('class' => 'btn btn-danger btn-xs', 'id' => 'btnDetalle', 'onclick' => 'recalcular();')) !!}   
...
{!! Form::button('<i class="glyphicon glyphicon-print"></i> Movilidad', array('class' => 'btn btn-warning btn-xs', 'id' => 'btnDetalle', 'onclick' => 'imprimirMovilidad();')) !!}  
	...
	<tbody>
		...
		<tr style="{{ $color }}" title="{{ $titulo }}">
            ...
            @else
                ...
                @endif
            @endif 
            <td align="center">{{ $value->numeroficha}}</td>
            <td align="center">{{ $value->nombrepaciente}}</td>
            ...
            @elseif($value->conceptopago_id<>2 && $value->conceptopago_id<>1 && $value->situacion<>'A' && $value->conceptopago_id<>60)
                <td align="center">{!! Form::button('<div class="glyphicon glyphicon-print"></div>', array('onclick' => 'imprimirRecibo ('.$value->id.');', 'class' => 'btn btn-xs btn-warning', 'title' => 'Imprimir')) !!}</td>
            @else
                <td align="center"> - </td>
            @endif
            <?php //echo $value->conceptopago_id.''; ?>
            @if($conceptopago_id<>2 && $value->situacion<>'A' && $value->conceptopago_id<>15 && $value->conceptopago_id<>17 && $value->conceptopago_id<>19 && $value->conceptopago_id<>21 && $value->conceptopago_id<>32 && $value->conceptopago_id<>60)
