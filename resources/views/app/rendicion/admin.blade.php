				...
				<div class="box-header">
					<div class="row">
						<div class="col-xs-12">
							...
							{!! Form::button('<i class="glyphicon glyphicon-file"></i> Pdf', array('class' => 'btn btn-success btn-xs', 'id' => 'btnBuscar', 'onclick' => 'pdf(\''.$entidad.'\')')) !!}
							{!! Form::close() !!}
						</div>
					</div>
				</div>
...
<script>
	...
	function pdf(entidad){
	    window.open("rendicion/pdf?fechainicial="+$(IDFORMBUSQUEDA + '{!! $entidad !!} :input[name="fechainicial"]').val()+"&fechafinal="+$(IDFORMBUSQUEDA + '{!! $entidad !!} :input[name="fechafinal"]').val()+"&persona="+$(IDFORMBUSQUEDA + '{!! $entidad !!} :input[name="nombre"]').val()+"&_token="+$(IDFORMBUSQUEDA + '{!! $entidad !!} :input[name="_token"]').val(),"_blank");
	}
</script>