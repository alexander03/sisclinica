...
class VentaController extends Controller
{
    ...
    public function agregarconvenio(Request $request)
    {
        ...
        return $convenio->nombre.'|'.$convenio->id;
    }
    ...
    public function create2(Request $request)
    {
        ...
        $entidad  = 'Venta';
        $venta = null;
        $tiposdocumento = Tipodocumento::where('tipomovimiento_id','=','4')->orderBy("nombre")->get();
        $cboCredito        = array("N" => 'NO', 'S' => 'SI');
        $cboTipoTarjeta    = array("VISA" => "VISA", "MASTER" => "MASTER");
        $cboTipoTarjeta2    = array("CREDITO" => "CREDITO", "DEBITO" => "DEBITO");
        return view($this->folderview.'.mant2')->with(compact('venta', 'entidad','tiposdocumento','cboCredito','cboFormapago','cboTipoTarjeta','cboTipoTarjeta2'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $error = DB::transaction(function() use($request, $sucursal_id ,&$dat){
            ...
            $validar = Venta::where('serie','=','2')->where('manual','like','N')->where('tipodocumento_id','=',$request->input('documento'))->where('numero','=',$request->input('numerodocumento'))->where('sucursal_id', $sucursal_id)->where('caja_id', $caja->id)->where('tipomovimiento_id', '4')->first();
            if ($validar == null) { 
                $ind = 0;
                $montoafecto = 0;
                $montonoafecto = 0;
                ...
                if ($ind == 0) {
                    ...
                    $venta->comentario='Pago de : '.$abreviatura.'002-'.$request->input('numerodocumento');
                    $venta->save();
                    ...
                    if ($request->input('formapago') == 'P') {
                        ...
                        if ( ($request->input('documento') == 15 && $venta->copago > 0 ) || ($request->input('documento') != 15)) {
                            ...
                            $movimiento->comentario='Pago de : '.$abreviatura.'002-'.$request->input('numerodocumento');
                    ...
                    $venta2->comentario='Pago de : '.$abreviatura.'002-'.Movimiento::NumeroSigue($caja->id, $sucursal_id, 4,$request->input('documento'),4,'N');
                ...
                if ($ind == 0) {
                    ...
                    $dat[0]=array("respuesta"=>"OK","venta_id"=>$venta->id, "ind" => $ind, "second_id" => 0, "guia" => $guia, "tipodocumento_id" => $venta->tipodocumento_id, 'numero' => $abreviatura.'002-'.$request->input('numerodocumento'));
                }else{
                    /*
                    if ($montoafecto > 0) {
                        if ($venta->conveniofarmacia_id !== null) {
            ...
            foreach($lista as $key => $value){
                if ( ($value->tipodocumento_id == 5) || ($value->tipodocumento_id == 4)) {
                    if ($guia == 'SI') {
                        ...
                        $pdf::Cell(20,5.5,("Prec. Unit."),1,0,'C');
                        $pdf::Cell(20,5.5,("Dscto"),1,0,'C');
                        $pdf::Cell(20,5.5,("Copago"),1,0,'C');
                        $pdf::Cell(20,5.5,("Total"),1,0,'C');
                        ...
                        foreach($lista2 as $key2 => $v){
                            ...
                            $pdf::Cell(20,5.5,number_format($v->precio,2,'.',''),0,0,'C');
                            ...
                            $pdf::Cell(20,5.5,number_format($dscto,2,'.',''),0,0,'C');
                            $pdf::Cell(20,5.5,number_format($value->copago,2,'.',''),0,0,'C');
                        ...
                        $pdf::SetFont('helvetica','B',9);
                        $pdf::Cell(115,5.5,'',0,0,'C');
                        $pdf::Cell(20,5.5,number_format($totalpago,2,'.',''),0,0,'C');
                ...
                foreach($lista as $key => $value){
                    if ( ($value->tipodocumento_id == 5) || ($value->tipodocumento_id == 4)) {
                        if ($guia == 'SI') {
                            ...
                            $pdf::SetFont('helvetica','B',9);
                            $pdf::Cell(20,6,("CANTIDAD"),0,0,'C');
                            $pdf::Cell(80,6,"PRODUCTO",0,0,'L');
                            $pdf::Cell(22,6,("PRECIO UNI"),0,0,'C');
                            $pdf::Cell(15,6,("DSCTO"),0,0,'C');
                            $pdf::Cell(20,6,("COPAGO"),0,0,'C');
                            $pdf::Cell(20,6,("TOTAL"),0,0,'C');
                            $pdf::Cell(20,6,("SIN IGV"),0,0,'C');
                            ...
                            foreach($lista2 as $key2 => $v){
                                ...
                                $pdf::Cell(22,6,number_format($v->precio,2,'.',''),0,0,'C');
                                $valaux = round(($v->precio*$v->cantidad), 2);
                                $precioaux = $v->precio - ($v->precio*($value->descuentokayros/100));
                                $dscto = round(($precioaux*$v->cantidad),2);
                                $totalpago = $totalpago+$dscto;
