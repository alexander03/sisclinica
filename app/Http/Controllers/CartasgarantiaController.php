...
class CartasgarantiaController extends Controller
{
    ...
    public function word(Request $request){
        ...
        $resultado        = $resultado->select('movimiento.*','cie.codigo as cie10',DB::raw('concat(paciente.apellidopaterno,\' \',paciente.apellidomaterno,\' \',paciente.nombres) as paciente'),'responsable.nombres as responsable','plan.razonsocial as empresa','plan.direccion as direccion2')->orderBy('plan.razonsocial','ASC')->orderBy('movimiento.fecha', 'ASC')->orderBy('movimiento.numero','ASC');
        ...
        foreach ($lista as $key => $value) {
            if($plan_id!=$value->plan_id){
                ...
                $section->addText('"AÑO DEL DIALOGO Y RECONCIALICION NACIONAL"',
