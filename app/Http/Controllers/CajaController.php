...
class CajaController extends Controller
{
    ...
    public function buscar(Request $request)
    {
        ...
        $apertura_caja              = Movimiento::where('tipomovimiento_id','=',2)->where('caja_id','=',$caja_id)->where('conceptopago_id','=',1)->orderBy('id','DESC')->limit(1)->first();
        if(count($rst)>0){
            $movimiento_mayor = $apertura_caja->id;    
        }else{
            $movimiento_mayor = 0;
        }
        if($movimiento_mayor != 0){
            $cierre_anterior              = Movimiento::where('tipomovimiento_id','=',2)->where('caja_id','=',$caja_id)->where('id','<=',$movimiento_mayor)->where('conceptopago_id','=',2)->orderBy('id','DESC')->limit(1)->first();
        }else{
            $cierre_anterior = 0;
        }
        ...
        if ($caja_id == 3 || $caja_id == 4) {
            $resultado2        = Movimiento::leftjoin('person as paciente', 'paciente.id', '=', 'movimiento.persona_id')
                            ...
                            ->where('movimiento.sucursal_id','=',$sucursal_id)
                            ->where('movimiento.id', '>=', $movimiento_mayor);
            $resultado2        = $resultado2->select('movimiento.*','m2.situacion as situacion2',DB::raw('case when paciente.bussinesname is null then concat(paciente.apellidopaterno,\' \',paciente.apellidomaterno,\' \',paciente.nombres) else paciente.bussinesname end as paciente'),DB::raw('responsable.nombres as responsable'))->orderBy('movimiento.id', 'desc');
        ...
        if (count($lista) > 0) {
            ...
            return view($this->folderview.'.list')->with(compact('lista', 'apertura_caja', 'sucursal_id', 'paginacion', 'cierre_anterior', 'inicio', 'fin', 'entidad', 'cabecera', 'titulo_modificar', 'titulo_eliminar', 'ruta', 'conceptopago_id', 'titulo_registrar', 'titulo_apertura', 'titulo_cierre', 'titulo_ticketspendientes', 'titulo_cuentaspendientes', 'ingreso', 'egreso', 'titulo_anular', 'garantia', 'efectivo', 'visa', 'master', 'listapendiente', 'user', 'tipousuario', 'caja_id'));
        }
    ...
    public function pdfDetalleCierreExcel(Request $request) {
        ...
        $caja_anterior = Movimiento::leftjoin('movimiento as m2','movimiento.movimiento_id','=','m2.id')
        ...
        $caja_anterior = $caja_anterior->select('movimiento.situacion','movimiento.voucher','movimiento.formapago','movimiento.comentario','movimiento.fecha','movimiento.numero','movimiento.total','movimiento.totalpagado','movimiento.totalpagadovisa','movimiento.totalpagadomaster','m2.numero as numeroticket',DB::raw('case when paciente.bussinesname is null then concat(paciente.apellidopaterno,\' \',paciente.apellidomaterno,\' \',paciente.nombres) else paciente.bussinesname end as paciente'), 'conceptopago.nombre as nombreconcepto', 'movimiento.total')->orderBy('movimiento.id', 'desc');

        $caja_anterior = $caja_anterior->limit(1)->get();
           
        //apertura caja

        $apertura_caja = Movimiento::leftjoin('movimiento as m2','movimiento.movimiento_id','=','m2.id')
        ->leftjoin('person as paciente', 'paciente.id', '=', 'movimiento.persona_id')
        ->join('conceptopago','conceptopago.id','=','movimiento.conceptopago_id')
        ->where('movimiento.tipomovimiento_id', '=', 2)
        ...
        $apertura_caja = $apertura_caja->select('movimiento.situacion','movimiento.voucher','movimiento.formapago','movimiento.comentario','movimiento.fecha','movimiento.numero','movimiento.total','movimiento.totalpagado','movimiento.totalpagadovisa','movimiento.totalpagadomaster','m2.numero as numeroticket',DB::raw('case when paciente.bussinesname is null then concat(paciente.apellidopaterno,\' \',paciente.apellidomaterno,\' \',paciente.nombres) else paciente.bussinesname end as paciente'), 'conceptopago.nombre as nombreconcepto', 'movimiento.total')->orderBy('movimiento.id', 'desc');

        $apertura_caja = $apertura_caja->limit(1)->get();
        ...
        Excel::create('ExcelReporte', function($excel) use($nomcierre2, $sucursal_id, $apertura_caja, $caja_anterior, $listaventas, $listacuotas, $listaventasfarmacia, $listaingresosvarios, $listaegresos, $listacajaanterior, $listaegresoscompra, $nomcierre, $responsable, $caja, $request) {
 
            $excel->sheet("Det. Cierre " . $nomcierre, function($sheet) use($nomcierre2, $sucursal_id, $apertura_caja, $caja_anterior, $listaventas, $listacuotas, $listaventasfarmacia, $listaingresosvarios, $listaegresos, $listacajaanterior, $listaegresoscompra, $nomcierre, $responsable, $caja, $request) {
                ...
                if(count($caja_anterior)>0){
                    ...
                    $sheet->cells('A'.$a.':F'.$a, function ($cells) {
                        ...
                        $cells->setBackground('#FFE003');
                    });
                    $a++;
                    $subtotalefectivo = 0;
                    $subtotalvisa = 0;
                    $subtotalmaster = 0;
                    foreach ($caja_anterior as $row) {                
                        $fila[] = date("d-m-Y", strtotime($row['fecha']));
                        $fila[] = $row['paciente'];
                        $fila[] = "";
                        $fila[] = "";
                        $fila[] = "";
                        $fila[] = $row['nombreconcepto'];
                        $fila[] = "";
                        $fila[] = "";
                        $fila[] = $row['total'];
                        $monto_cierre = $row['total'];
                        $sheet->row($a, $fila);
                        $a++;
                        $fila = array();    
                    } 
                    ...
                    $fila[] = '';                           
                    $fila[] = '';                           
                    $fila[] = number_format($monto_cierre,2,'.','');  
                    $fila[] = ''; 
                    $fila[] = '';                          
                    $sheet->row($a, $fila);
                ...
                $monto_apertura = 0;
                if($sucursal_id == 2){
                    if(count($apertura_caja)>0){
                        $a++;
                        $b = $a;
                        $sheet->row($a, array('APERTURA DE CAJA'));
                        $sheet->mergeCells('A'.$a.':F'.$a);
                        $sheet->cells('A'.$a.':F'.$a, function ($cells) {
                            $cells->setFont(array(
                                'family'     => 'Calibri',
                        ...
                        $fila[] = '';                           
                        $fila[] = '';                           
                        $fila[] = number_format($monto_apertura,2,'.','');  
                        $fila[] = ''; 
                        $fila[] = '';                          
                        $sheet->row($a, $fila);
                        $sheet->mergeCells('A'.$a.':H'.$a); 
                ...
                if(count($listaegresos)>0){
                    ...
                    foreach ($listaegresos as $row) { 
                        ...
                        if($row['situacion'] == 'N') {
                            ...
                            $fila[] = '';
                            $sheet->mergeCells('I'.$a.':K'.$a);
                            $sheet->cells('I'.$a.':K'.$a, function($cell){
                ...
                $fila[] = '';
                $fila[] = 'SALDO (S/.)';
                $fila[] = number_format($monto_apertura + $totalefectivo + $totalmaster + $totalvisa - $subtotalegresos,2,'.','');
                $sheet->row($a, $fila);
                $fila = array();
                $a++;
                ...
                $fila[] = '';
                $fila[] = 'CAJA (S/.)';
                $fila[] = number_format($monto_apertura + $totalefectivo-$subtotalegresos,2,'.','');
                $sheet->row($a, $fila);
                ...
                if($sucursal_id == 2){
                    $sheet->setBorder('E'.($a-15).':F'.$a, 'thin');                 
                    $sheet->cells('E'.($a-15).':F'.$a, function ($cells) {
                        $cells->setFont(array(
                            'family'     => 'Calibri',
                            'size'       => '11',
                            'bold'       =>  true
    ...
    public function pdfDetalleCierreExcelF(Request $request) {
        ...
        $listaventasfarmacia = Movimiento::leftjoin('movimiento as m2','movimiento.movimiento_id','=','m2.id')
                ...
                ->whereBetween('movimiento.fecha', [$fi, $ff])
                ->where('movimiento.ventafarmacia', '=', 'S');
        $listaventasfarmacia = $listaventasfarmacia->select('paciente.id as pid','movimiento.situacion','movimiento.doctor_id','movimiento.serie','movimiento.id','movimiento.nombrepaciente','movimiento.voucher','movimiento.formapago','movimiento.comentario','movimiento.fecha','movimiento.numero','movimiento.total','movimiento.totalpagado','movimiento.totalpagadovisa','movimiento.totalpagadomaster','m2.numero as numeroticket',DB::raw('case when paciente.bussinesname is null then concat(paciente.apellidopaterno,\' \',paciente.apellidomaterno,\' \',paciente.nombres) else paciente.bussinesname end as paciente'), 'movimiento.total')->orderBy('movimiento.numero', 'asc');
        ...
        Excel::create('ExcelReporte', function($excel) use($listaventas, $listacuotas, $listaventasfarmacia, $listaingresosvarios, $listaegresos, $listaegresoscompra, $nomcierre, $caja, $responsable, $request) {
            ...
            $excel->sheet("Det. Cierre " . $nomcierre, function($sheet) use($listaventas, $listacuotas, $listaventasfarmacia, $listaingresosvarios, $listaegresos, $listaegresoscompra, $caja, $nomcierre, $responsable, $request) {
                ...
                $sheet->setWidth(array(
                    'A' => 15,'B' => 40, 'C' => 5, 'D' => 10, 'E' => 35, 'F' => 50, 'G' => 10, 'H' => 10, 'I' => 10, 'J' => 10, 'K' => 10, 'L' => 15, 'M' => 40
                ));
                ...
                $sheet->cells('A1:' . $hasta . '3', function ($cells) {
                    $cells->setFont(array(
                        'family'     => 'Calibri',
                        'size'       => '11',
                        'bold'       =>  true
                ...
                if(count($listaventasfarmacia)>0){
                    ...
                    foreach ($listaventasfarmacia as $row) { 
                        ...
                        if($mov !== NULL) {
                            ...
                            $hist = Historia::where('person_id', '=', $row['pid'])->first();
                            if($hist!==NULL) {                                        
                                $fila[] = ($hist->tipopaciente=='Particular'?'PARTICULAR':'CONVENIO') . ': ' . ($hist->convenio==NULL?' - ':$hist->convenio->nombre);
                            } else {
                                $fila[] = 'EXTERNO: -';
                            }
                            if($row['situacion'] == 'N') { 
        ...
        Excel::create('ExcelReporte', function($excel) use($aperturas, $numcajas, $nomcierre, $nomcierre2, $caja, $responsable, $request) {
            ...
            if(count($aperturas) > 0) {
                foreach ($aperturas as $apertura) {
                    ...
                    ->where('movimiento.tipomovimiento_id', '=', 2)
                    ->where('movimiento.conceptopago_id', '=', 2)
                    ->where('movimiento.id', '<', $apertura->id)
                    ->where('movimiento.sucursal_id', '=', $sucursal_id)
                    ->where('movimiento.caja_id', '=', $caja_id);
                    ...
                    $excel->sheet($nomcierre . ' Ap. ' . $apertura->numero, function($sheet) use($listaventas, $sucursal_id, $apertura_caja, $caja_anterior, $listacuotas, $listaventasfarmacia, $listaingresosvarios, $listacajaanterior, $listaegresos, $listaegresoscompra, $caja, $nomcierre, $nomcierre3, $responsable, $request, $apertura) {
                        ...
                        $sheet->cells('A2:' . $hasta . '3', function ($cells) {
                            $cells->setFont(array(
                                'family'     => 'Calibri',
                                'size'       => '11',
                                'bold'       =>  true
                            ...
                            foreach ($listaventasfarmacia as $row) { 
                                ...
                                if($mov !== NULL) {
                                    ...
                                    if($row['situacion'] == 'N') {
                                        ...
                                        if($valuetpv == 0){$valuetpv='';}
                                        if($valuetpm == 0){$valuetpm='';}  
                                        $fila[] = $row['descuento'] . ($row['tipodescuento']=='P'?'%':'');
                                        $fila[] = $valuetp;                           
                                        $fila[] = $valuetpv;                           
                                        $fila[] = $valuetpm;
                                    } else {
                        ...
                        $fila[] = '';
                        $fila[] = 'VENTAS DEL DÍA';
                        $fila[] = number_format($totalefectivo + $totalmaster + $totalvisa - $monto_apertura,2,'.','');
                        $sheet->row($a, $fila);
                        $sheet->cells('E'.$a.':F'.$a, function ($cells) {
                            $cells->setFont(array(
                                'bold'       =>  true
                            ));
                            $cells->setBackground('#D5F5E3');
                        });
                        $fila = array();
                        $a++;
                        ...
                        $fila[] = '';
                        $fila[] = 'Efectivo';
                        $fila[] = number_format($totalefectivo - $monto_apertura,2,'.','');
                        $sheet->row($a, $fila);
                        $fila = array();
                        $a++;
                        ...
                        $fila[] = '';
                        $fila[] = 'SALDO (S/.)';
                        $fila[] = number_format($monto_apertura + $totalefectivo + $totalmaster + $totalvisa - $subtotalegresos - $monto_apertura,2,'.','');
                        $sheet->row($a, $fila);
                        $fila = array();
                        $a++;
                        ...
                        $fila[] = '';
                        $fila[] = 'CAJA (S/.)';
                        $fila[] = number_format($monto_apertura + $totalefectivo-$subtotalegresos - $monto_apertura,2,'.','');
                        $sheet->row($a, $fila);
    ...
    public function pdfDetalleCierre(Request $request){
        ...
        if($sucursal_id == 2) {
            ...
            if(count($listaingresosvarios)>0){
                $pdf::SetFont('helvetica','B',8.5);
                $pdf::Cell(267,7,'APERTURA DE CAJA',1,0,'L');
                $pdf::Ln();
                $subtotalefectivo = 0;
                $subtotalvisa = 0;
                $subtotalmaster = 0;
        ...
        $pdf::Ln();
        $pdf::Cell(120,7,utf8_decode(""),0,0,'C');
        $pdf::Cell(30,7,utf8_decode("CAJA ANTERIOR :"),1,0,'L');
        $pdf::Cell(20,7,number_format($cierre_anterior->total,2,'.',''),1,0,'R');
        $pdf::Ln();
        if($sucursal_id == 2){
            $pdf::Cell(120,7,utf8_decode(""),0,0,'C');
            $pdf::Cell(30,7,utf8_decode("APERTURA CAJA:"),1,0,'L');
            $pdf::Cell(20,7,number_format($monto_apertura,2,'.',''),1,0,'R');
            $pdf::Ln();
        }
        $pdf::Cell(120,7,utf8_decode(""),0,0,'C');
        $pdf::Cell(30,7,utf8_decode("INGRESOS :"),1,0,'L');
        $pdf::Cell(20,7,number_format($totalefectivo + $totalmaster + $totalvisa - $monto_apertura,2,'.',''),1,0,'R');
        $pdf::Ln();
        $pdf::Cell(120,7,utf8_decode(""),0,0,'C');
        $pdf::Cell(30,7,utf8_decode("Efectivo :"),1,0,'L');
        $pdf::Cell(20,7,number_format($totalefectivo - $monto_apertura,2,'.',''),1,0,'R');
        $pdf::Ln();
        $pdf::Cell(120,7,utf8_decode(""),0,0,'C');
        $pdf::Cell(30,7,utf8_decode("Master :"),1,0,'L');
...
*/
    ...
    public function pdfDetalleCierreF2(Request $request){
        ...
        if($sucursal_id == 2) {
            ...
            $pdf::Cell(120,7,utf8_decode(""),0,0,'C');
            $pdf::Cell(30,7,utf8_decode("CAJA ANTERIOR :"),1,0,'L');
            $pdf::Cell(20,7,number_format($caja_anterior,2,'.',''),1,0,'R');
            $pdf::Ln();
            if($sucursal_id == 2){
                $pdf::Cell(120,7,utf8_decode(""),0,0,'C');
                $pdf::Cell(30,7,utf8_decode("APERTURA CAJA:"),1,0,'L');
    ...
    public function pdfDetalleCierreOriginal(Request $request){
        ...
        /*if ($caja_id != 6) {
            $resultado2        = Movimiento::leftjoin('person as paciente', 'paciente.id', '=', 'movimiento.persona_id')
                            ...
                            ->whereNull('movimiento.cajaapertura_id');
            $resultado2        = $resultado2->select('movimiento.*','m2.situacion as situacion2',DB::raw('CONCAT(paciente.apellidopaterno," ",p,paciente.apellidomaterno," ",paciente.apellidomaterno," ",paciente.nombres) as paciente'),DB::raw('responsable.nombres as responsable'))->orderBy('movimiento.id', 'desc');
            $listapendiente            = $resultado2->get();
        }*/
        if (isset($lista)) { 
            $sucursal_id = Session::get('sucursal_id'); 
        ...
        if($lista->conceptopago_id==10){//GARANTIAS
            ...
            if($lista->conceptopago->id=="6" || $lista->conceptopago->id=="7" || $lista->conceptopago->id=="8"){
                ...
                for($c=0;$c<count($list);$c++){
                    ...
                    $pdf::Cell(75,7,($detalle->servicio==null?$detalle->descripcion:$detalle->servicio->nombre),1,0,'C');
        ...
        if (count($lista) > 0 || count($lista2) > 0) {            
            ...
            foreach ($lista as $key => $value){
                ...
                for($c=0;$c<count($list);$c++){
                    ...
                    if($detalle->servicio_id>0){
                        $pdf::Cell(75,6,$detalle->servicio->nombre,1,0,'L');
                    }else{
                        $pdf::Cell(75,6,$detalle->descripcion,1,0,'L');
                    }
                    ...
                    if($movcaja['situacion'] != 'A') {
                        ...
                        if(count($detalles) > 0) { 
                            ...
                            foreach ($detalles as $detalle) {
                                ...
                                $pdf::Cell(60,7,substr(($detalle->servicio==null?$detalle->descripcion:$detalle->servicio->nombre),0,40) . '.',1,0,'L');  
                                $pdf::Cell(14,7,number_format($detalle->precio,2,',',''),1,0,'R');
                                if($a == 0) {
                    ...
                    if(count($movimientoscuotas) > 0) {
                        ...
                        if(count($detalles) > 0) {
                            ...
                            foreach ($detalles as $detalle) {
                                ...
                                $pdf::Cell(60,7,($detalle->servicio==null?substr($detalle->descripcion,0,40):substr($detalle->servicio->nombre,0,40)) . '.',1,0,'L');                                  
                                $pdf::Cell(14,7,number_format($detalle->precio,2,',',''),1,0,'R');
                                if($a == 0) {
    ...
    public function reporteCiruProSinDetalle(Request $request) {
        ...
        foreach ($rs as $value) {
            ...
            foreach ($detalles as $detalle) {
                ...
                if($cont == 1){
                    $tipo.= $cont . " - " . ($servicio==null?$detalle->descripcion:$servicio->nombre);
                    if($detalle->tipodescuento == "P"){
                    ...
                    $tipo.= "\n\n" . $cont . " - " . ($servicio==null?$detalle->descripcion:$servicio->nombre);
                   if($detalle->tipodescuento == "P"){
    ...
    public function pdfDetalleEgresos(Request $request) {
        ...
        Excel::create('ExcelReporte', function($excel) use($egresos, $aperturas, $numcajas, $caja_id, $sucursal_id, $nomcierre) {
            ...
            $excel->sheet("Det. Egres. " . $nomcierre, function ($sheet) use ($egresos, $aperturas, $numcajas, $caja_id, $sucursal_id) {
    ...
    public function pdfDetallePorProductoF(Request $request){
        setlocale(LC_TIME, 'spanish');
        $caja    = Caja::find($request->input('caja_id'));
        $caja_id = Libreria::getParam($request->input('caja_id'),'1');
        ...
        Excel::create('ExcelDetPorProd', function($excel) use($caja, $caja_id, $fi, $ff, $user, $responsable, $responsable_nombre, $totalvisa, $totalmaster, $totalefectivo, $totalegresos, $subtotalegresos, $sucursal_id, $nomcierre, $request) {
            $excel->sheet('ExcelDetPorProd', function($sheet) use($caja, $caja_id, $fi, $ff, $user, $responsable, $responsable_nombre, $totalvisa, $totalmaster, $totalefectivo, $totalegresos, $subtotalegresos, $sucursal_id, $nomcierre, $request) {

                $sheet->setWidth(array(
                    'A' => 15,'B' => 40, 'C' => 30, 'D' => 5, 'E' => 10, 'F' => 40, 'G' => 15, 'H' => 15, 'I' => 15, 'J' => 15, 'K' => 25
                ));
                ...
                $cabecera1 = array();
                $cabecera1[] = 'Detalle Cierre por Producto de ' . $nomcierre;
                $sheet->row($indicee,$cabecera1);
                ...
                $listaventasfarmacia = Movimiento::leftjoin('movimiento as m2','movimiento.movimiento_id','=','m2.id')
                        ...
                        ->whereBetween('movimiento.fecha', [$fi, $ff])
                        ->where('movimiento.ventafarmacia', '=', 'S');
                $listaventasfarmacia = $listaventasfarmacia->select('movimiento.persona_id', 'movimiento.situacion','movimiento.doctor_id','movimiento.serie','movimiento.id','movimiento.nombrepaciente','movimiento.voucher','movimiento.formapago','movimiento.comentario','movimiento.fecha','movimiento.numero','movimiento.total','movimiento.totalpagado','movimiento.totalpagadovisa','movimiento.totalpagadomaster','m2.numero as numeroticket',DB::raw('case when paciente.bussinesname is null then concat(paciente.apellidopaterno,\' \',paciente.apellidomaterno,\' \',paciente.nombres) else paciente.bussinesname end as paciente'), 'movimiento.total')->orderBy('movimiento.numero', 'asc');
                ...
                if(count($listaventasfarmacia)>0){
                    ...
                    foreach ($listaventasfarmacia as $row) { 
                        ...
                        foreach ($detalleproductos as $value) {
                            ...
                            $hist = Historia::where('person_id', '=', $row['persona_id'])->first();
                            if($hist!==NULL) {                                        
                                $fila[] = ($hist->tipopaciente=='Particular'?'PARTICULAR':'CONVENIO') . ': ' . ($hist->convenio==NULL?' - ':$hist->convenio->nombre);
                            } else {
                                $fila[] = 'EXTERNO: -'.$row['persona_id'];
                            }               
                            $fila[] = ($mov==null?'G/R':$mov->tipodocumento->abreviatura);
                            $fila[] = utf8_decode($row['serie'] . '-' . $row['numero']);
                            $fila[] = $producto;
                            $fila[] = utf8_decode(" - ");
                            if($row['situacion'] == 'N') {
                                $fila[] = utf8_decode("EMITIDO");
                            } else {
                                $fila[] = utf8_decode("ANULADO");
                            } 
                            if($row['situacion'] == 'N') {
                                $valuetp = number_format($row['totalpagado'],2,'.','');
                                $valuetpv = number_format($row['totalpagadovisa'],2,'.','');
                    ...
                    $sheet->setBorder('A3:K'.$a, 'thin');

                    $sheet->mergeCells('A'.($a).':I'.($a));
                    $sheet->cells('A'.$a.':K'.$a, function ($cells) {
                        $cells->setFont(array(
                            'family'     => 'Calibri',
                            'size'       => '11',
                            'bold'       =>  true
                    ...
                    $fila[] = 'RESUMEN DE CAJA';
                    $sheet->mergeCells('F'.$a.':G'.$a);
                    $sheet->cells('F'.$a.':G'.$a, function ($cells) {
                        $cells->setFont(array(
                            'family'     => 'Calibri',
                            'size'       => '11',
                            'bold'       =>  true
                    ...
                    $sheet->row($a, $fila);
                    $fila = array();
                    $sheet->cells('F'.($a-3).':G'.$a, function ($cells) {
                        $cells->setFont(array(
                            'bold'       =>  true
                        ));
                        $cells->setBackground('#FCF3CF');
                    ...
                    $sheet->cells('F'.($a-2).':G'.$a, function ($cells) {
                        $cells->setFont(array(
                            'bold'       =>  true
                        ));
                        $cells->setBackground('#F9E79F');
                    ...
                    $sheet->cells('F'.($a-10).':G'.$a, function ($cells) {
                        $cells->setFont(array(
                            'family'     => 'Calibri',
                            'size'       => '11',
                            'bold'       =>  true
                    ...
                    $sheet->setBorder('F'.($a-10).':G'.$a, 'thin');

                    $sheet->cells('G'.($a-10).':G'.$a, function ($cells) {
                        $cells->setFont(array(
                            'size'       => '11',
                        ));
                    });
    ...
    public function pdfDetallePorProductoAgrupado(Request $request){
        ...
        if(count($listaventasfarmacia)>0){
            foreach ($listaventasfarmacia as $key => $row) {
                ...
                $pdf::Cell(125,7,utf8_encode(utf8_decode($row['nombre'])),1,0,'L');
    ...
    public function pdfDetallePorLoteStockFV(Request $request){        
        ...
        if($almacen_id == 1) { $caja_id = 3;} 
        elseif($almacen_id == 2) { $caja_id = 1;}
        elseif($almacen_id == 3) { $caja_id = 4;}
        elseif($almacen_id == 4) { $caja_id = 2;}
        ...
        if($producto_id !== '') {
            $productos = $productos->where('producto.id', '=', $producto_id);
        }
    ...
    public function mostrarDetallePago(Request $request)
    {
        $doctor_id = $request->input('doctor_id');
        $fechainicial = $request->input('fechainicial');
        $fechafinal = $request->input('fechafinal');

        $medico = Person::find($doctor_id);
        ...
        $pdf::SetFont('helvetica','B',15);
        $pdf::Cell(0,10,'REPORTE DE PAGO OFTALMOLOGIA ('. $medico->apellidopaterno . " " . $medico->apellidomaterno . " " . $medico->nombres. "): " . date("d/m/Y", strtotime($fechainicial)) . " - " .  date("d/m/Y", strtotime($fechafinal)) ,0,0,'C');
        $pdf::Ln();
        ...
        $pdf::SetFont('helvetica','B',9);
        $pdf::Cell(50,7,utf8_decode("RESUMEN DE PAGOS"),1,0,'C');
        $pdf::Cell(20,7,utf8_decode("MONTO"),1,0,'C');
        $pdf::Cell(20,7,utf8_decode("PAGO"),1,0,'C');
        $pdf::Ln();
        $pdf::Cell(100,7,utf8_decode(""),0,0,'C');
    ...
    public function editarformapago(Request $request) {
        ...
        $listar   = 'SI';
        $entidad  = 'Caja2';
        $ingreso  = Movimiento::where('id', '=', $id)->first();
        $formData = array('caja.editarformapago2');
        $formData = array('route' => $formData, 'class' => 'form-horizontal', 'id' => 'formMantenimiento'.$entidad, 'autocomplete' => 'off');
        $boton    = 'Cambiar'; 
        return view($this->folderview.'.editarformapago')->with(compact('ingreso', 'formData', 'entidad', 'boton', 'listar', 'id'));
    }

    public function editarformapago2(Request $request) {
        //$dat=array();
        //$error = DB::transaction(function() use($request,&$dat){
            $id = $request->input('id22');
