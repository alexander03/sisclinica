...
class TramaController extends Controller
{
    ...
    protected $tituloAdmin     = 'Generación de Trama';
    ...
    protected $rutas           = array('nueva' => 'trama.nueva', 
            'edit'   => 'trama.edit', 
            'delete' => 'trama.eliminar',
            'search' => 'trama.buscar',
            'generar' => 'trama.generar',
            'index'  => 'trama.index',
            'buscar' => 'trama.listarD',
            'listarD' => 'trama.listarD'
        );
    ...
    public function nueva(){
        ...
        return view($this->folderview.'.nueva')->with(compact('lote'));
    }
    ...
    public function listarD(Request $request){
        ...
        $resultado        = Movimiento::join('person as paciente','paciente.id','=','movimiento.persona_id')
                            ...
                            //->where('movimiento.numero','=',6334)
                            ->where('movimiento.fecha','>=',$fechainicial)
                            ->where('movimiento.fecha','<=',$fechafinal)
                            //->where('movimiento.responsable_id','=',$user->person_id)
                            ;
                            //LILIANA 31227
                            //MAYRA 57059
                            //JEANCARLOS 57058
        ...
        $errores = 0;
        $reporte_error = '';
        $response = '<table class="table table-hover"><thead><tr><th>Documento</th><th>Paciente</th><th>Siniestro</th><th>CIE10</th><th>Total</th><th></th></tr></thead><tbody>';
        $response1 = '';
        $response2 = '';
    ...
    function generarAte($lista,$periodo,$lote,$plan,$fecha,$ruc,$ipress,$v_iafa){
        ...
        $resultado        = Movimiento::join('person as paciente','paciente.id','=','movimiento.persona_id')
                            ...
                            ->whereIn('movimiento.id',$lista);
        $resultado        = $resultado->orderBy('movimiento.serie', 'ASC')->orderBy('movimiento.numero', 'ASC')
                            ->select('movimiento.id as movid','cie.codigo as cie10','movimiento.subtotal','movimiento.igv','movimiento.total',DB::raw('plan.nombre as plan'),'historia.carnet','movimiento.tipomovimiento_id','movimiento.serie','movimiento.numero',DB::raw('movimiento.fecha'),DB::raw('TIME(movimiento.created_at) as hora2'),'historia.tipopaciente','movimiento.copago',DB::raw('historia.numero as historia'),'movimiento.tarjeta','movimiento.tipotarjeta','movimiento.situacion as situacion2','historia.numero as historia','paciente.dni',DB::raw('responsable.nombres as responsable'),DB::raw('movimiento.numero as numero2'),'empresa.bussinesname','empresa.direccion','empresa.ruc','movimiento.comentario','movimiento.fechaingreso','movimiento.fechaalta','cie.descripcion as cie','movimiento.soat');
        $lista            = $resultado->get();
        $contador = 0;
        foreach ($lista as $key => $value){
        //if ($value->comentario != '' && $value->cie10 != '') {
            ...
            $blaboratorio = $blaboratorio->select('detallemovcaja.id','detallemovcaja.descripcion','servicio.tiposervicio_id','detallemovcaja.precio','detallemovcaja.cantidad');
    ...
    function generarFar($lista,$periodo,$lote,$plan,$fecha,$ruc,$ipress,$v_iafa){
        ...
        foreach ($lista as $key => $value){
            ...
            $txt = $txt."".$ruc."".$ipress."01".$v_numero."1".$v_correlativo."   OXXXXXXXXXXXXXXXO     ".$v_fecha."".$v_cantidadfar."".$v_sinigv."".$v_copagovariable."".$v_sinigv."".$v_montoneto."".$v_cie10."AXXXXXXXXXXXX";
            $contador++;
            fwrite($myfile, $txt);
        }
