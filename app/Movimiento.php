    ...
    public function scopeNumeroSigue($query,$caja_id,$sucursal_id,$tipomovimiento_id,$tipodocumento_id=0,$serie=0,$manual='S'){
        if($tipodocumento_id==0){
            ...
            if($serie==0){
                ...
                //QUITAMOS CAJA_ID PARA QUE SIGA EL CORRELATIVO ASI FACTURE CAJA OJOS CON RUC ESPECIALIDADES
                $rs=$query->where('sucursal_id','=',$sucursal_id)->where('tipomovimiento_id','=',$tipomovimiento_id)->where('manual','like',$manual)->where('tipodocumento_id','=',$tipodocumento_id)->where('serie','=',$serie)->select(DB::raw("max((CASE WHEN numero IS NULL THEN 0 ELSE numero END)*1) AS maximo"))->first();
